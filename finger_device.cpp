...
bool FingerDevice::initSDK() {
    ...
    if (!g_deviceSDK) {
        std::cout << "[INFO] 加载设备 SDK 失败: " << dlerror() << std::endl;
        return false;
    }
    ...
    if (!g_enumDevices || !g_openDevice || !g_closeDevice || 
        ...
        std::cout << "[INFO] 获取函数指针失败: " << dlerror() << std::endl;
        dlclose(g_deviceSDK);
        g_deviceSDK = nullptr;
        return false;
    }
...
void FingerDevice::destroySDK() {
    if (g_deviceSDK) {
        ...
        g_status = nullptr;
        std::cout << "[INFO] 设备 SDK 已销毁" << std::endl;
    }
}
...
bool FingerDevice::isDeviceConnected() {
    ...
    std::cout << "[INFO] 检测到 " << count << " 个设备" << std::endl;
    return count > 0;
}

bool FingerDevice::openDevice() {
    if (isOpen_) {
        std::cout << "[INFO] 设备已经打开" << std::endl;
        return false;
    }
    
    if (deviceList_.empty()) {
        std::cout << "[INFO] 没有可用设备" << std::endl;
        return false;
    }
        
    deviceHandle_ = g_openDevice(&deviceList_[0]);
    if (!deviceHandle_) {
        std::cout << "[INFO] 打开设备失败" << std::endl;
        return false;
    }
    ...
    if (status != 0) {
        std::cout << "[INFO] 设备状态异常: " << status << std::endl;
    ...
    isOpen_ = true;
    std::cout << "[INFO] 设备打开成功" << std::endl;
    return true;
}

bool FingerDevice::closeDevice() {
    if (!isOpen_) {
        std::cout << "[INFO] 设备未打开" << std::endl;
        return false;
    }
    ...
    if (result == 0) {
        isOpen_ = false;
        deviceHandle_ = nullptr;
        std::cout << "[INFO] 设备关闭成功" << std::endl;
        return true;
    }
    
    std::cout << "[INFO] 设备关闭失败: " << result << std::endl;
    return false;
}
...
int FingerDevice::captureImage(unsigned char* buffer, int size) {
    ...
    if (result <= 0) {
        std::cout << "[INFO] 采集图像失败: " << result << std::endl;
    }
    return result;
}
...
int FingerDevice::getParameter(int type) {
    ...
    if (type != 1 && type != 2) {
        std::cout << "[INFO] 无效的参数类型: " << type << std::endl;
        return -1;
    }
    
    int value = g_getParameter(deviceHandle_, type);
    if (value <= 0) {
        std::cout << "[INFO] 获取参数失败: type=" << type << ", result=" << value << std::endl;
    }
    return value;
}
